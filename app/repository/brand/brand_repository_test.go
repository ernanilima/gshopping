...
func TestFindAll_Should_Return_All_Brands(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"count", "id", "description", "created_at"}).
		AddRow(2, uuid.New(), "Marca para teste 1", time.Date(2021, time.January, 1, 21, 31, 41, 0, time.UTC)).
		AddRow(2, uuid.New(), "Marca para teste 2", time.Date(2022, time.February, 2, 22, 32, 42, 0, time.UTC))
...
func TestFindById_Should_Return_A_Brand_When_Searching_By_ID(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"id", "description", "created_at"}).
		AddRow(uuid.New(), "Marca para teste 1", time.Date(2021, time.January, 1, 21, 31, 41, 0, time.UTC))

	// cria um mock da query executada
	mock.ExpectQuery("SELECT (.+) FROM brand WHERE id").WillReturnRows(rows).RowsWillBeClosed()
	connector.EXPECT().OpenConnection().Return(db)
...
func TestFindAll_Should_Return_The_Brands_Found_When_Searching_By_Description(t *testing.T) {
	...
	// cria um mock da query executada
	mock.ExpectQuery("SELECT (.+) FROM brand WHERE description").WillReturnRows(rows).RowsWillBeClosed()
	connector.EXPECT().OpenConnection().Return(db)
