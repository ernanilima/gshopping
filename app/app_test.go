...
func TestApp(t *testing.T) {
	...
	t.Run("Deve retornar o status 200 e um produto localizado ao pesquisar por codigo de barras", func(t *testing.T) {
		// cria uma requisicao HTTP GET para "/v1/produto/codigo-barras/{barcode}"
		req, err := http.NewRequest("GET", "/v1/produto/codigo-barras/789102030", nil)
		...
		assert.Equal(t, model.Brand{Description: "1906"}, result.Brand)
	...
	t.Run("Deve retornar o status 404 ao nao localizar um produto pesquisado por codigo de barras", func(t *testing.T) {
		// cria uma requisicao HTTP GET para "/v1/produto/codigo-barras/{barcode}"
		req, err := http.NewRequest("GET", "/v1/produto/codigo-barras/789302010", nil)
		assert.NoError(t, err)
		// cria um HTTP recorder para receber a resposta
		res := httptest.NewRecorder()
		// executa a requisicao no router
		...
		assert.Equal(t, "/v1/produto/codigo-barras/789302010", result.Path)
	})
}
